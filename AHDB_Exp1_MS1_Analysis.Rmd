...
p <- ggplot(obs_death, aes(x = AgeCategory, y = Proportion_plot, fill = Sex)) +
  ...
  scale_fill_manual(values = c("purple", "#000000")) +
  scale_x_discrete(
...
# Plot
ggplot(Younger_females_Mito, aes(x = Died_InTrt, y = Blood_mtDNA.Mean)) +
...
# Plot
ggplot(Younger_females_Telo, aes(x = Died_InTrt, y = Blood_Telomeres.per.cell)) +
  geom_violin(aes(fill = Died_InTrt), trim = FALSE, alpha = 0.6) +  # Violin by group
  geom_jitter(width = 0.1, alpha = 0.7, size = 1.5, color = "black") +  # Add data points
  labs(
    x = "Died In Treatment?",
