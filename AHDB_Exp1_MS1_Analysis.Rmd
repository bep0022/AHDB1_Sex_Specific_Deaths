...
# Result: 5.2%%
# Result without removing unconscious: 0.05930259
```
...
p <- ggplot(obs_death_plot, aes(x = AgeCategory, y = Proportion_fixed, fill = Sex)) +
  geom_col(
...
# Results removing lysed : P = 0.235
# Results not removing lysed : P = 0.483
```
...
p4 <- ggplot(Younger_females_Mito, aes(x = Died_InTrt, y = Blood_mtDNA.Mean)) +
  geom_violin(aes(fill = Died_InTrt), trim = FALSE, alpha = 0.9) +  # Violin by group
  geom_jitter(
  width = 0.1,
  shape = 21,      # Circle with fill and outline
...
# Telomeres Plot
p5 <- ggplot(Younger_females_Telo, aes(x = Died_InTrt, y = Blood_Telomeres.per.cell)) +
  geom_violin(aes(fill = Died_InTrt), trim = FALSE, alpha = 0.9) +  # Violin by group
  geom_jitter(
  width = 0.1,
  shape = 21,      # Circle with fill and outline
...
# CORT Plot
p6 <- ggplot(Younger_females_CORT, aes(x = Died_InTrt, y = CORT.ng.mL.)) +
  geom_violin(aes(fill = Died_InTrt), trim = FALSE, alpha = 0.9) +  # Violin by group
  geom_jitter(
  width = 0.1,
  shape = 21,      # Circle with fill and outline
...
combined_blood_measure_plot <- plot_grid(p1, p2, p3, p4, p5, p6, nrow = 2)
combined_blood_measure_plot

#ggsave("Combined_Blood_ViolinPlot.png", combined_blood_measure_plot, width = 12, height = 4)
